...
const OWNER = "centurygolf";
const REPO = "2026-partners-invitational";
...
let parentSha = null;
try {
  parentSha = gh(["api", `repos/${OWNER}/${REPO}/git/ref/heads/${BRANCH}`]).object.sha;
  console.log("parent commit:", parentSha.slice(0, 7));
} catch {
  console.log("empty repo, writing the first commit");
}
...
if (parentSha) {
  gh(["api", `repos/${OWNER}/${REPO}/git/refs/heads/${BRANCH}`, "-X", "PATCH"], { sha: commit.sha });
} else {
...
/* 7. Pages, enabled on first run only. Skipped when the token lacks admin,
      which is the case while this account only grants Write. */
